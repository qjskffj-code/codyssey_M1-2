...
이 질문에 답하기 위해 공개 데이터 102개월치를 직접 모아, **그 데이터만 근거로 대답하는 AI 비서**를 만들었습니다.

<p align="center">
  <img src="assets/images/코디세이_M1-2_웹서비스_01_메인화면_핵심인사이트_260921.png" alt="대전 관광 AI 비서 메인 화면" width="950">
</p>
...
<p align="center">
  <a href="https://codyssey-m1-2-theta.vercel.app"><b>서비스 열기</b></a> ·
  <a href="https://codyssey-m1-2-vix0.onrender.com/docs"><b>API 문서(Swagger)</b></a> ·
  <a href="#05-직접-써보기">직접 써보기</a> ·
  <a href="#12-공식-미션-요구사항-체크리스트">미션 요구사항 체크리스트</a>
</p>
...
문서가 깁니다. 목적에 따라 필요한 곳만 보셔도 됩니다.

| 시간 | 읽을 곳 | 알 수 있는 것 |
|---|---|---|
| **3분** | [01 한눈에 보기](#01-한눈에-보기) → [04 화면](#04-화면과-기능) → [05 직접 써보기](#05-직접-써보기) | 무엇을 만들었고 어떻게 쓰는지 |
| **10분** | 위 + [02 데이터 이야기](#02-데이터-이야기) + [03 동작 원리](#03-동작-원리) | 왜 이 결론이 나왔고 어떻게 동작하는지 |
| **전체** | 06번 이후 전부 | 구조·API·배포·테스트·의사결정 |
| **채점** | [12 미션 체크리스트](#12-공식-미션-요구사항-체크리스트) | 요구사항 충족 여부 |

처음 보는 분은 **01 → 02 → 05** 순서를 권합니다.

### 목차

| | 섹션 | 내용 |
|---|---|---|
| 01 | [한눈에 보기](#01-한눈에-보기) | 무엇을 만들었나, 기술 스택, 용어 |
| 02 | [데이터 이야기](#02-데이터-이야기) | 어떤 데이터를 썼고 무엇을 발견했나 |
| 03 | [동작 원리](#03-동작-원리) | 컨텍스트 주입이란 무엇인가 |
| 04 | [화면과 기능](#04-화면과-기능) | 스크린샷과 시연 GIF |
| 05 | [직접 써보기](#05-직접-써보기) | 3분 체험 시나리오 |
| 06 | [프로젝트 구조](#06-프로젝트-구조) | 폴더와 역할 분담 |
| 07 | [API 명세](#07-api-명세) | 엔드포인트 11개 |
| 08 | [로컬 실행](#08-로컬-실행) | 내 컴퓨터에서 띄우기 |
| 09 | [배포](#09-배포) | Render · Vercel 설정 |
| 10 | [데이터 파이프라인](#10-데이터-파이프라인) | 수집 → 정제 → 적재 |
| 11 | [오류 처리와 테스트](#11-오류-처리와-테스트) | 예외 상황과 검증 결과 |
| 12 | [미션 요구사항 체크리스트](#12-공식-미션-요구사항-체크리스트) | 요구사항 대조표 |
| 13 | [문제 해결 기록](#13-문제-해결-기록) | 막힌 것 9건과 해결 과정 |
| 14 | [의사결정과 회고](#14-의사결정과-회고) | 왜 그렇게 만들었나 |
...
일반 챗봇에게 "요즘 대전 관광객 어때?"라고 물으면 **일반론**이 돌아옵니다. 제 데이터를 모르기 때문입니다.

이 서비스는 다릅니다. 질문을 받으면 먼저 **저장된 102개월치 데이터의 요약을 GPT에게 읽힌 뒤** 답하게 합니다.
그래서 "2020년에 왜 줄었어?"라고 물으면 실제 수치와 그달에 있었던 일을 근거로 답합니다.

| 질문 | 일반 챗봇 | 이 서비스 |
|---|---|---|
| 2020년에 방문자가 줄어든 이유가 뭐야? | "코로나19 때문일 수 있습니다" (추측) | "2020-02 코로나19 대구 집단감염, 2020-03 거리두기 시행 시기로 **각각 185,964명, 165,843명**까지 감소했습니다" (데이터 인용) |
| 성심당 검색량과 방문자는 관계있어? | "관련이 있을 수 있습니다" | "**2018-01~2026-06, 102개월** 기준 r=0.471의 뚜렷한 양의 상관입니다. 다만 인과는 아닙니다" |
...
| 데이터 주제 | 대전 방문자 수(이동통신 빅데이터) ↔ "성심당" 검색 관심도 |
| 데이터 규모 | 월별 **102개월** (2018-01 ~ 2026-06), 일별 원본 23,195건을 집계 |
| 핵심 흐름 | 공공데이터 수집 → 월별 정제 → Firestore 적재 → 요약 계산 → 프롬프트 주입 → GPT 답변 → 대화 자동 저장 |
| 핵심 발견 | 성심당 검색 관심 ↔ 대전 방문자 **r = 0.471**, 외국인 방문자 **r = 0.784** |
| 배포 | 백엔드 Render · 프론트엔드 Vercel (GitHub 푸시 시 자동 재배포) |
| 보너스 과제 | 추가 지표 3종 · SVG 그래프 · CSV 내보내기 · 다크 모드 (4/4 완료) |
...
> ⏳ **첫 접속은 30초~1분 걸릴 수 있습니다.**
> 백엔드가 무료 서버라 15분간 요청이 없으면 잠듭니다. 화면을 열면 자동으로 서버를 깨우고 안내 문구를 띄웁니다.

## 이 문서에 자주 나오는 말

처음 보는 용어가 있다면 여기를 참고하세요.

| 용어 | 뜻 |
|---|---|
| **성심당** | 대전의 대표 베이커리. 2023년 이후 전국적으로 화제가 되며 대전 관광의 상징처럼 언급됩니다 |
| **외지인** | 대전에 거주하지 않는 방문자. 이 프로젝트의 주 지표입니다 (관광객에 가장 가까운 값) |
| **검색 지수** | 네이버에서 "성심당"을 검색한 양. 실제 횟수가 아니라 **가장 많았던 달을 100으로 둔 상대값**입니다 |
| **상관계수 (r)** | 두 숫자가 함께 움직이는 정도. +1이면 같이 오르고, 0이면 무관, -1이면 반대로 움직입니다 |
| **컨텍스트 주입** | GPT에게 질문하기 전에 내 데이터 요약을 먼저 읽히는 것. 이 프로젝트의 핵심입니다 |
| **콜드스타트** | 무료 서버가 절전 상태에서 깨어나느라 첫 응답이 느려지는 현상 |
...
데이터 한 줄은 **한 달**을 나타냅니다. 이렇게 생겼습니다.
...
| `junggu_share` | 대전 5개 구 방문자 중 중구(성심당 본점 소재) 비중 | % | 2024-01 ~ |
| `memo` | 그달에 있었던 사건 | 텍스트 | 해당 월만 |

> 💡 **왜 "하루 평균"인가?**
> 원본이 일별 데이터라 달마다 일수가 달라(28~31일) 월 합계로 비교하면 2월이 불리해집니다. 하루 평균으로 맞추면 달끼리 공정하게 비교됩니다.

> 💡 **값이 없는 달은 0이 아니라 비워 둡니다.**
> 0으로 채우면 AI가 "2019년에 외국인 방문자가 0명이었다"고 잘못 답합니다.

## 무엇을 발견했나

처음 가설은 "성심당 인기 ↑ → 대전 관광객 ↑"였습니다.
계산해 보니 **어떤 데이터를 쓰느냐에 따라 결론이 달라졌습니다.**

| 주 지표로 쓴 데이터 | 성심당 검색 지수와의 상관계수 | 해석 |
|---|---:|---|
...
**① 담는 장소가 달랐습니다.** 입장객 통계는 국립중앙과학관, 한밭교육박물관, 화폐박물관 같은 **시설 위주**입니다. 성심당이 있는 원도심의 방문 흐름과는 다른 이야기를 하고 있었습니다.

**② 집계 대상이 중간에 바뀌었습니다.** 2023년 12월을 끝으로 8곳(계족산 황톳길, 뿌리공원, 한밭수목원 등)이 보고에서 빠졌습니다. 그래서 숫자만 보면 2024년에 관광객이 반토막 난 것처럼 보입니다.

| 연도 | 30개 관광지 입장객 (연간) | 성심당 검색 지수 (12개월 합) |
|---|---:|---:|
| 2022 | 10,225,887명 | 250 |
| 2023 | 10,659,448명 | 545 |
| **2024** | **5,747,011명** ← 실제 감소 아님 | 782 |

**실제로 관광객이 줄어든 게 아니라 세는 대상이 줄어든 것**이었습니다. 그래서 주 지표를 이동통신 기반 방문자 수로 바꿨고, 상관계수가 -0.05에서 +0.47로 바뀌었습니다.

### 장소로도 확인했습니다

성심당 본점이 있는 **중구만 다른 구보다 빠르게 성장**했습니다. 서구·유성구가 대조군 역할을 합니다.
...
**월 단위 등락은 함께 움직이지 않습니다.** 검색량은 뉴스 이슈가 터지면 급등하지만, 방문자는 천천히 변합니다.
실제로 2024-05에 임대료 논란으로 검색이 최고치(100)를 찍었지만 그달 방문자는 평소와 비슷했습니다.

또한 두 지표 모두 시간이 지나며 증가하는 추세라, **공통 추세 때문에 상관계수가 부풀려졌을 가능성**도 있습니다.

그래서 이 서비스는 항상 이렇게 답하도록 설계했습니다.

> "상관관계는 있지만 인과관계로 단정할 수 없습니다."
...
위쪽은 **데이터를 준비하는 과정**(한 번만 실행), 아래쪽은 **사용자가 질문할 때마다 도는 과정**입니다.

## 컨텍스트 주입이란

식당에 비유하면 이렇습니다.

> 손님(사용자)이 주문하기 전에, 요리사(GPT)에게 **오늘 들어온 재료 목록**을 먼저 보여주는 것.
> 재료를 모르면 "파스타 맛있어요" 같은 일반론만 말하지만, 재료를 알면 "오늘 바지락이 좋으니 봉골레를 추천합니다"라고 말할 수 있습니다.

`POST /api/chat`은 다음 4단계로 동작합니다.

| 단계 | 하는 일 |
|---|---|
| 1 | `analytics.build_summary()`로 요약 계산 |
| 2 | 요약을 시스템 프롬프트에 삽입 |
| 3 | 직전 대화 최대 10개와 함께 GPT 호출 (응답 토큰 500 제한) |
...
**① 사건 목록을 넣는 것**
숫자만 주면 AI는 "왜"를 답하지 못합니다. 사건을 넣은 뒤에야 원인을 설명했습니다.

| | 같은 질문 "2020년에 방문자가 줄어든 이유가 뭐야?" |
|---|---|
| 사건 주입 **전** | "2020년 방문자 수 감소 원인은 주어진 데이터에 포함되어 있지 않습니다." |
| 사건 주입 **후** | "코로나19 국내 확산(2020-02)과 사회적 거리두기 시행(2020-03) 시기에 해당하며, 해당 월 방문자 수가 각각 185,964명과 165,843명으로 크게 줄었습니다." |

**② 집계 기간을 함께 넣는 것**
외국인 데이터는 2020년부터만 있습니다. 기간을 넣지 않으면 AI가 임의의 기간을 말합니다.

**③ 한계를 함께 넣는 것**
"중복 집계된다", "관심 지표다", "상관은 인과가 아니다"를 넣으면 AI가 스스로 단서를 붙여 답합니다.
**데이터 서비스에서는 잘 말하게 만드는 것보다 틀리지 않게 만드는 것이 중요**하다고 판단했습니다.
...
카드를 나열하는 대신, 맨 위에 **결론 한 문장과 핵심 수치 3개**를 놓았습니다.
...
파란 선이 방문자 수, 주황 선이 성심당 검색 지수입니다. **사건이 있던 달은 점과 라벨로 표시**됩니다.
상관계수는 대표 지표를 크게, 보조 지표를 작게 배치해 무엇이 중요한지 드러냈습니다.

<p align="center">
  <img src="assets/images/코디세이_M1-2_웹서비스_02_추이그래프_지표관계_260921.png" alt="사건 주석이 표시된 이중 축 그래프와 상관계수" width="950">
</p>

차트 라이브러리를 쓰지 않고 SVG 좌표를 직접 계산했습니다. 라벨이 겹치면 자동으로 생략합니다.

## 데이터 관리

102건을 추가·수정·삭제할 수 있고, CSV로 내보낼 수 있습니다.
...
| 2 | 추이 그래프 | 이중 축 SVG 차트, 사건 주석, 기간 필터(전체/최근 3년/코로나 이후), 툴팁 |
| 3 | 데이터 관리 | 추가 · 인라인 수정 · 삭제, CSV 내보내기(엑셀 한글 깨짐 방지) |
...
<https://codyssey-m1-2-theta.vercel.app> 에 접속한 뒤 순서대로 해보시면 3분 안에 전체 기능을 볼 수 있습니다.

| 순서 | 해볼 것 | 확인할 점 |
|---|---|---|
| 1 | 접속 직후 기다리기 | "서버를 깨우는 중" 안내가 뜹니다 (무료 서버 콜드스타트 대응) |
| 2 | 상단 인사이트 밴드 보기 | 결론과 핵심 수치 3개가 먼저 보입니다 |
| 3 | 그래프에 마우스 올리기 | 해당 월의 방문자 수·검색 지수·사건이 표시됩니다 |
| 4 | 기간 필터 `최근 3년` 클릭 | 36개월로 좁혀지며 최근 변화가 잘 보입니다 |
| 5 | **추천 질문 "2020년에 방문자가 줄어든 이유가 뭐야?" 클릭** | **사건 메모를 근거로 답합니다 ← 이 프로젝트의 핵심** |
| 6 | 이어서 "성심당 검색량과 방문자는 관계가 있어?" 질문 | 상관계수와 계산 기간을 말하고, 인과로 단정하지 않습니다 |
| 7 | `데이터 관리` 탭 → `+ 데이터 추가` | 연월 형식이나 범위가 틀리면 즉시 오류 메시지가 뜹니다 |
| 8 | 아무 행의 `수정` 클릭 | 인라인 편집으로 바뀌고, 저장하면 그래프와 요약이 함께 갱신됩니다 |
| 9 | `기록` 버튼 클릭 | 이전 대화를 불러와 이어서 질문할 수 있습니다 |
| 10 | 오른쪽 위 🌙 클릭 | 다크 모드로 전환됩니다 |

> 5번과 6번이 가장 중요합니다. 일반 챗봇과 이 서비스의 차이가 드러나는 지점입니다.
...
주문받는 사람과 요리하는 사람을 나눈 것과 같습니다.

| 폴더 | 역할 | 예시 |
|---|---|---|
| `routers/` | HTTP 경계. 요청을 받고 상태 코드로 응답 | 없는 데이터면 404 반환 |
| `services/` | 실제 계산과 저장. HTTP를 모름 | 상관계수 계산, Firestore 조회 |

덕분에 챗봇이 요약을 쓸 때 **자기 API를 HTTP로 다시 부르지 않고** `analytics.build_summary()`를 직접 호출합니다.
네트워크 왕복이 사라지고, 같은 로직을 스크립트에서도 재사용할 수 있습니다.

> 🔒 `.env`, `.venv/`, 서비스 계정 JSON은 `.gitignore`로 제외했고, 커밋 전 `git check-ignore`로 실제 제외 여부를 확인했습니다.
...
> ⚠️ `/api/data/summary`는 `/api/data/{id}`보다 **먼저 선언**해야 `summary`가 ID로 해석되지 않습니다.

## 요약 API 응답

이 응답이 그대로 GPT의 프롬프트 재료가 됩니다.
...
각 상관계수에 **`period`와 `count`가 붙어 있는 점**이 핵심입니다. 지표마다 집계 기간이 다르기 때문입니다.
...
<http://localhost:5500> 에서 접속합니다. `localhost`로 열면 자동으로 로컬 백엔드를 바라봅니다.
...
**Secret File로 업로드한 뒤 경로만 환경 변수로 전달**하면 깔끔합니다.
...
> ⚠️ 배포 후 **반드시** Render의 `ALLOWED_ORIGINS`에 Vercel 도메인을 추가합니다.
...
| 데이터 | 출처 | 알아둘 점 |
|---|---|---|
| 방문자 수 | [공공데이터포털 한국관광공사 빅데이터 지역별 방문자수](https://www.data.go.kr/data/15101972/openapi.do) | KT(내국인)·SKT(외국인) 이동통신 기반 **추정치**. 2박 3일 머물면 3명으로 집계되므로 절대량이 아니라 추세로 읽어야 함 |
| 검색 지수 | [네이버 데이터랩 검색어 트렌드](https://datalab.naver.com/keyword/trendSearch.naver) | 실제 검색 횟수가 아니라 **최댓값 100 기준 상대 지표**. 방문이 아니라 관심을 나타냄 |
| 관광지점 입장객 | [관광지식정보시스템](https://know.tour.go.kr/stat/visitStatDis/table.do) | 비교 검증용으로만 사용. 집계 대상 관광지가 시기별로 바뀌어 시계열로 쓰기 어려움 |
...
> 위 항목은 **데이터 인용과 기간 명시 여부**를 기준으로 판단했습니다.
...
미션 3번은 **"아래를 스스로 설명할 수 있어야 한다"**를 요구합니다. 각 항목이 이 문서의 어디에 있는지 정리했습니다.

| 과제 목표 | 이 문서의 위치 |
|---|---|
| 시계열 분석 → 요약 정보 → 서비스 활용 흐름 | [02 데이터 이야기](#02-데이터-이야기), [10 데이터 파이프라인](#10-데이터-파이프라인) |
| FastAPI 라우터/서비스 분리 기준 | [06 프로젝트 구조](#06-프로젝트-구조) |
| Pydantic 검증 이유와 방식 | [11 오류 처리](#11-오류-처리와-테스트) |
| Firestore 저장과 CRUD | [07 API 명세](#07-api-명세), [14 의사결정](#14-의사결정과-회고) |
| 컨텍스트 주입 원리 | [03 동작 원리](#03-동작-원리) |
| 배포 환경의 CORS·환경변수·키 관리 | [09 배포](#09-배포), [13 문제 해결 기록](#13-문제-해결-기록) |

---

# 13 문제 해결 기록

실제로 막혔던 9건입니다. 증상 → 원인 → 해결 순서로 정리했습니다.
...
**해결** — `events` 필드를 추가해 사건 목록을 주입했습니다. ([03 동작 원리](#03-동작-원리)의 전후 비교 참고)
