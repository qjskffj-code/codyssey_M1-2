...
> "성심당이 유명해지면 대전 관광객도 늘까?" — 공개 데이터 102개월치를 직접 모아, 그 데이터만 근거로 답하는 AI 비서입니다.
...
<p align="center">
  <img src="assets/images/코디세이_M1-2_웹서비스_01_메인화면_핵심인사이트_260921.png" alt="대전 관광 AI 비서 메인 화면" width="900">
</p>
...
| 구현 형태 | FastAPI 백엔드 + 바닐라 JavaScript 프론트엔드 웹 서비스 |
...
- 프론트엔드가 진입 시 `/health`를 먼저 호출해 서버를 깨우고 안내 문구를 표시합니다.
...
- 일반 챗봇은 **내 데이터를 모릅니다.** "요즘 대전 관광객 어때?"에 일반론을 답합니다.
- 이 서비스는 질문을 받으면 **저장된 102개월치 요약을 GPT에게 먼저 읽힙니다.**
- 그래서 수치와 기간을 인용해 답하고, 근거가 없으면 없다고 말합니다.

**같은 질문, 다른 답변**

| 질문 | 일반 챗봇 | 이 서비스 |
|---|---|---|
| 2020년에 방문자가 줄어든 이유는? | "코로나19 때문일 수 있습니다" | "2020-02 코로나19 대구 집단감염, 2020-03 거리두기 시행 시기로 각각 **185,964명 · 165,843명**까지 감소했습니다" |
| 성심당 검색량과 방문자는 관계있나? | "관련이 있을 수 있습니다" | "**2018-01~2026-06, 102개월** 기준 r=0.471의 뚜렷한 양의 상관입니다. 다만 인과는 아닙니다" |
...
- **성심당** — 대전의 대표 베이커리. 2023년 이후 전국적으로 화제가 되며 대전 관광의 상징처럼 언급됩니다.
- **외지인** — 대전에 거주하지 않는 방문자. 관광객에 가장 가까운 값이라 주 지표로 썼습니다.
- **검색 지수** — 네이버에서 "성심당"을 검색한 양. 실제 횟수가 아니라 가장 많았던 달을 100으로 둔 상대값입니다.
- **상관계수 (r)** — 두 숫자가 함께 움직이는 정도. +1이면 같이 오르고, 0이면 무관, -1이면 반대로 움직입니다.
- **컨텍스트 주입** — GPT에게 질문하기 전에 내 데이터 요약을 먼저 읽히는 것. 이 프로젝트의 핵심입니다.
- **콜드스타트** — 무료 서버가 절전에서 깨어나느라 첫 응답이 느려지는 현상.
- **CORS** — 브라우저가 "이 사이트가 저 서버를 불러도 되는가"를 확인하는 보안 절차.
...
데이터 기반 AI 서비스를 만들며 실제로 부딪힌 문제입니다.

- 공식 입장객 통계는 집계 대상이 시기마다 바뀌어, 시계열로 쓰면 **없는 급감이 만들어집니다.**
- 지표마다 제공 기간이 달라(2018 / 2020 / 2024 시작) 하나의 상관계수로 말하면 오해를 부릅니다.
- 방문자 수는 계절성이 강해 직전 3개월과 비교하면 **계절 변동을 추세로 착각**합니다.
- 요약에 숫자만 넣으면 AI가 "왜 줄었는지"를 답하지 못합니다.
- 요약에 기간을 빼면 AI가 **기간을 임의로 추측**합니다.
...
| 문제 | 해결 방법 |
|---|---|
| 집계 대상 변동으로 인한 가짜 급감 | 주 지표를 이동통신 기반 방문자 수로 교체, 입장객 통계는 비교 검증용으로만 사용 |
| 지표별 제공 기간 차이 | 상관계수마다 계산 구간과 표본 수를 함께 응답, 값 없는 달은 0이 아니라 필드 생략 |
| 계절성으로 인한 추세 왜곡 | 추세를 전년 동기 대비로 계산하고 비교한 두 구간을 응답에 포함 |
| AI가 원인을 설명하지 못함 | memo를 사건 목록(`events`)으로 만들어 프롬프트에 함께 주입 |
| AI의 과장·단정 | "중복 집계", "관심 지표", "상관≠인과"를 주의사항으로 주입 |
| 서비스 계정 키 관리 | Render Secret File로 업로드하고 경로만 환경 변수로 전달 |
...
```json
{
  ...
  "foreign_value": 2860,
  "junggu_share": 19.4,
  "memo": "임대료 논란 확산, 성심당 검색 지수 최고치"
}
```

| 필드 | 의미 | 단위 | 제공 기간 |
|---|---|---|---|
| `date` | 연월 (문서 ID로도 사용) | YYYY-MM | 2018-01 ~ 2026-06 |
| `value` | 대전을 방문한 **외지인 수** — 그달의 하루 평균 | 명/일 | 102개월 전체 |
| `ref_value` | **"성심당" 검색 지수** — 최대 월을 100으로 환산 | 0~100 | 102개월 전체 |
| `foreign_value` | 대전을 방문한 외국인 수 — 하루 평균 | 명/일 | 2020-01 ~ |
| `junggu_share` | 대전 5개 구 방문자 중 중구(성심당 본점) 비중 | % | 2024-01 ~ |
| `memo` | 그달에 있었던 사건 | 텍스트 | 해당 월만 |

**설계에서 지킨 두 가지**

- 원본이 일별이라 달마다 일수가 달라, **하루 평균으로 맞춰** 달끼리 공정하게 비교합니다.
- 값이 없는 달은 **0이 아니라 비워 둡니다.** 0으로 채우면 AI가 "2019년 외국인 0명"이라고 답합니다.

## 데이터를 바꾸니 결론이 뒤집혔습니다

처음 가설은 "성심당 인기 ↑ → 대전 관광객 ↑"였습니다.

| 주 지표로 쓴 데이터 | 검색 지수와의 상관계수 | 해석 |
|---|---:|---|
| 관광지점 입장객 통계 (공식 집계) | **-0.05** | 관계 없음 |
| 대전 **외지인** 방문자 (이동통신) | **+0.47** | 뚜렷한 양의 상관 |
| 대전 **외국인** 방문자 (이동통신) | **+0.78** | 강한 양의 상관 |

**공식 통계로는 관계가 보이지 않은 이유**

- 담는 장소가 달랐습니다 — 과학관·박물관 등 **시설 위주**라 원도심 방문 흐름을 담지 못했습니다.
- 집계 대상이 중간에 바뀌었습니다 — 2023년 12월을 끝으로 **8곳이 보고에서 빠졌습니다.**
- 그래서 숫자만 보면 2024년에 관광객이 반토막 난 것처럼 보입니다.

| 연도 | 30개 관광지 입장객 (연간) | 검색 지수 (12개월 합) |
|---|---:|---:|
| 2022 | 10,225,887명 | 250 |
| 2023 | 10,659,448명 | 545 |
| **2024** | **5,747,011명** ← 실제 감소 아님 | 782 |

## 장소로도 확인했습니다

성심당 본점이 있는 중구만 다른 구보다 빠르게 성장했습니다.

| 구 | 2024년 (명/일) | 2025년 (명/일) | 증가율 |
|---|---:|---:|---:|
...
- 서구·유성구가 대조군 역할을 합니다.
- 중구만 9.9%로 다른 구(5~6%)를 앞섭니다.

## 그래도 단정하지 않습니다

- **월 단위 등락은 함께 움직이지 않습니다.** 검색은 뉴스로 튀고, 방문자는 천천히 변합니다.
- 실제로 2024-05에 검색이 최고치(100)를 찍었지만 그달 방문자는 평소와 비슷했습니다.
- 두 지표 모두 증가 추세라 **공통 추세가 상관을 부풀렸을 가능성**도 있습니다.
- 그래서 서비스는 항상 "상관은 있지만 인과로 단정할 수 없다"고 답하도록 설계했습니다.
...
- 위쪽은 데이터를 준비하는 과정입니다. 한 번만 실행합니다.
- 아래쪽은 사용자가 질문할 때마다 도는 과정입니다.

## 컨텍스트 주입이란

- 식당에 비유하면 **주문 전에 요리사에게 오늘 들어온 재료 목록을 보여주는 것**입니다.
- 재료를 모르면 일반론만 말하지만, 알면 오늘 재료로 추천할 수 있습니다.
...
- **사건 목록** — 숫자만 주면 "왜"를 답하지 못합니다.
- **집계 기간** — 넣지 않으면 AI가 임의의 기간을 말합니다.
- **해석 한계** — 넣으면 AI가 스스로 단서를 붙여 답합니다.

**사건 주입 전후 비교** (질문: "2020년에 방문자가 줄어든 이유가 뭐야?")
...
| | 기능 | 설명 |
|---|---|---|
| 1 | 핵심 인사이트 밴드 | 결론 한 문장 + 최근 방문자 · 전년 동기 대비 · 대표 상관계수 |
| 2 | 추이 그래프 | 이중 축 SVG 차트, 사건 주석, 기간 필터, 툴팁 |
| 3 | 데이터 관리 | 추가 · 인라인 수정 · 삭제, CSV 내보내기 |
| 4 | AI 채팅 | 추천 질문 칩, 타이핑 로딩 표시, 대화 이어가기 |
| 5 | 대화 기록 | 목록 조회, 불러오기, 삭제 |
| 6 | 다크 모드 | 브라우저 설정 자동 감지 + 선택 저장 |

## 1. 핵심 인사이트 밴드

- 카드를 나열하는 대신 결론 한 문장을 맨 위에 둡니다.
- 최근 방문자, 전년 동기 대비, 대표 상관계수 세 수치를 함께 보여줍니다.

## 2. 추이 그래프 (인라인 SVG)

- 왼쪽 축은 방문자 수(천 명), 오른쪽 축은 검색 지수(0~100)입니다.
- memo가 있는 달은 점과 라벨로 표시하고, 라벨 간격이 좁으면 자동 생략합니다.
- 기간 필터는 전체 / 최근 3년 / 코로나 이후 세 가지입니다.
- 마우스를 올리면 가장 가까운 달의 수치와 사건을 보여줍니다.
- 차트 라이브러리 없이 `path`, `line`, `text` 좌표를 직접 계산했습니다.

## 3. 데이터 관리 (CRUD)

- 추가 — 연월 형식(`YYYY-MM`), 0 이상 방문자 수, 0~100 검색 지수를 Pydantic이 검증합니다.
- 수정 — 행을 인라인 편집 폼으로 전환해 값과 메모를 고칩니다.
- 삭제 — 확인 후 삭제하고, 즉시 그래프와 요약을 다시 계산합니다.
- 내보내기 — BOM을 포함한 CSV라 엑셀에서 한글이 깨지지 않습니다.

## 4. AI 채팅

- 추천 질문 칩을 누르면 바로 질문이 전송됩니다.
- 답변을 기다리는 동안 타이핑 애니메이션을 표시합니다.
- `conversation_id`를 유지해 후속 질문에서 맥락이 이어집니다.

## 5. 대화 기록

- `기록` 버튼으로 목록을 열고, 클릭하면 메시지를 복원합니다.
- 목록 API는 본문을 제외해 가볍게 응답합니다.
- 본문은 단건 조회에서 받습니다.

## 6. 다크 모드

- CSS 변수로 색상 토큰을 정의하고 `data-theme` 속성으로 전환합니다.
- 첫 방문 시 브라우저 설정을 따릅니다.
- 이후 선택은 `localStorage`에 저장합니다.
...
- 파란 선이 방문자 수, 주황 선이 검색 지수입니다.
- 사건이 있던 달은 점과 라벨로 표시됩니다.
- 상관계수는 대표 지표를 크게, 보조 지표를 작게 배치했습니다.
...
<https://codyssey-m1-2-theta.vercel.app> 에서 순서대로 해보면 3분 안에 전체 기능을 볼 수 있습니다.

1. 접속 직후 기다리기 — "서버를 깨우는 중" 안내가 뜹니다.
2. 상단 인사이트 밴드 — 결론과 핵심 수치 3개가 먼저 보입니다.
3. 그래프에 마우스 올리기 — 해당 월의 방문자 수·검색 지수·사건이 표시됩니다.
4. 기간 필터 `최근 3년` — 36개월로 좁혀집니다.
5. **추천 질문 "2020년에 방문자가 줄어든 이유가 뭐야?" 클릭** — 사건 메모를 근거로 답합니다.
6. **이어서 "성심당 검색량과 방문자는 관계가 있어?" 질문** — 상관계수와 기간을 말하고 인과로 단정하지 않습니다.
7. `데이터 관리` 탭 → `+ 데이터 추가` — 형식이 틀리면 즉시 오류 메시지가 뜹니다.
8. 아무 행의 `수정` — 인라인 편집으로 바뀌고, 저장하면 그래프와 요약이 갱신됩니다.
9. `기록` 버튼 — 이전 대화를 불러와 이어서 질문할 수 있습니다.
10. 오른쪽 위 🌙 — 다크 모드로 전환됩니다.

> 5번과 6번이 핵심입니다. 일반 챗봇과의 차이가 드러나는 지점입니다.
...
- `routers/` — HTTP 경계입니다. 요청을 받고 상태 코드로 응답합니다.
- `services/` — 실제 계산과 저장입니다. HTTP를 모릅니다.
- 덕분에 챗봇이 자기 API를 HTTP로 다시 부르지 않고 `analytics.build_summary()`를 직접 호출합니다.

**보안**

- `.env`, `.venv/`, 서비스 계정 JSON은 `.gitignore`로 제외했습니다.
...
- 대화 불러오기는 미션의 (A)안을 선택했습니다. 목록은 가볍게, 본문은 단건 조회로 받습니다.
...
- 이 응답이 그대로 GPT 프롬프트의 재료가 됩니다.
- 각 상관계수에 `period`와 `count`가 붙어 있는 것이 핵심입니다.
...
실행 정책으로 활성화가 제한되면 현재 터미널에만 다음을 적용합니다.
...
- 서비스 계정 JSON은 **파일 전체 경로**를 지정합니다.
- 폴더 경로만 넣으면 실행 시 `PermissionError`가 발생합니다.
- 실제 키는 README, 캡처, 채팅, Git 커밋 어디에도 넣지 않습니다.
...
- <http://localhost:5500> 으로 접속합니다.
- `localhost`로 열면 `config.js`가 자동으로 로컬 백엔드를 바라봅니다.
...
- 서비스 계정 키는 줄바꿈이 포함된 JSON이라 환경 변수 한 줄로 넣기 번거롭습니다.
- Secret File로 업로드한 뒤 경로만 환경 변수로 전달하면 깔끔합니다.
...
- 정적 사이트라 별도 빌드가 없으므로 `outputDirectory`를 `.`로 고정해야 합니다.
- 빌드 단계에서 환경 변수를 `config.js`에 주입하므로 코드에 주소가 박히지 않습니다.
...
| `FIREBASE_SERVICE_ACCOUNT_PATH` | 둘 중 하나 | 서비스 계정 JSON 경로 |
...
- 발급처에서 즉시 폐기하고 재발급합니다.
- Render·Vercel 환경 변수를 교체하고 재배포합니다.
- 커밋 이력에 남았다면 히스토리에서 제거한 뒤 강제 푸시합니다.
...
| 데이터 | 출처 | 알아둘 점 |
|---|---|---|
| 방문자 수 | [공공데이터포털 한국관광공사 빅데이터 지역별 방문자수](https://www.data.go.kr/data/15101972/openapi.do) | 이동통신 기반 **추정치**. 2박 3일 머물면 3명으로 집계되므로 추세로만 읽어야 함 |
| 검색 지수 | [네이버 데이터랩 검색어 트렌드](https://datalab.naver.com/keyword/trendSearch.naver) | 실제 검색 횟수가 아니라 **최댓값 100 기준 상대 지표**. 방문이 아니라 관심을 나타냄 |
| 관광지점 입장객 | [관광지식정보시스템](https://know.tour.go.kr/stat/visitStatDis/table.do) | 비교 검증용으로만 사용. 집계 대상이 시기별로 바뀌어 시계열로 쓰기 어려움 |
...
전부 500으로 뭉뚱그리면 사용자가 무엇을 고쳐야 할지 모릅니다. 상황별로 구분했습니다.
...
| 서버 절전(콜드스타트) | — | `/health` 3회 재시도 + "서버를 깨우는 중 (최대 1분)" |
...
- AI 답변은 모델 상태와 실행 시점에 따라 문장이 달라질 수 있습니다.
- 위 항목은 데이터 인용과 기간 명시 여부를 기준으로 판단했습니다.
...
미션 3번은 "아래를 스스로 설명할 수 있어야 한다"를 요구합니다.

| 과제 목표 | 이 문서의 위치 |
|---|---|
| 시계열 분석 → 요약 정보 → 서비스 활용 흐름 | [Data Story](#data-story), [Data Pipeline](#data-pipeline) |
| FastAPI 라우터/서비스 분리 기준 | [Project Structure](#project-structure) |
| Pydantic 검증 이유와 방식 | [Features](#features), [Error Handling](#error-handling) |
| Firestore 저장과 CRUD | [API](#api), [Key Decisions](#key-decisions) |
| 컨텍스트 주입 원리 | [Architecture](#architecture) |
| 배포 환경의 CORS·환경변수·키 관리 | [Deployment](#deployment), [Troubleshooting](#troubleshooting--changes) |
...
- **원인** — 2023년 12월을 끝으로 계족산 황톳길, 뿌리공원, 한밭수목원 등 8곳이 집계에서 빠졌습니다.
- **해결** — 주 지표를 이동통신 기반 방문자 수로 교체하고, 입장객 통계는 비교 검증용으로만 사용했습니다.
- **결과** — 상관계수가 -0.05에서 +0.47로 바뀌었습니다.

## 2. 계절성이 추세로 오해되는 문제

- **증상** — "최근 3개월이 직전 3개월 대비 -10.5% 감소"로 표시됐습니다.
- **원인** — 방문자 수는 봄·가을에 몰리는 계절성이 강합니다.
- **해결** — 전년 동기 대비로 변경해 -4.4%로 보정했습니다.
- **추가** — 비교한 두 구간(`2026-04~06` vs `2025-04~06`)을 응답에 함께 담았습니다.

## 3. AI가 기간을 임의로 추측하는 문제

- **증상** — 추세를 설명하며 "(2024-04 ~ 2026-06 기준)"처럼 실제와 다른 구간을 말했습니다.
- **원인** — 요약에 비교 구간이 없어 AI가 추측했습니다.
- **해결** — 추세 문구에 비교 구간을 명시했습니다. 이후 정확한 구간을 인용했습니다.
...
- **결과** — 코로나19 확산과 거리두기 시행을 수치와 함께 설명하게 됐습니다.

## 5. 사건 메모의 시점 오류

- **증상** — 성심당 임대료 논란을 2024-04에만 기록했습니다.
- **확인** — 수집한 검색 지수의 최고치는 2024-05(=100)였고 4월은 67이었습니다.
- **해결** — 보도 시작(2024-04)과 화제 정점(2024-05)을 나눠 기록하고, 타결(2024-09)을 추가했습니다.
- **추가** — 전체 메모를 정책브리핑·보건복지부·대전시 등 1차 출처로 재검증했습니다.

## 6. Firestore 인증 오류

- **증상** — `PermissionError: [Errno 13] Permission denied: 'E:/codyssey/M1-2'`
- **원인** — `FIREBASE_SERVICE_ACCOUNT_PATH`에 폴더 경로만 입력했습니다.
- **해결** — JSON 파일명까지 포함한 전체 경로로 수정했습니다.
- **확인** — 키 파일이 `.gitignore`로 제외되는 것을 `git check-ignore`로 확인했습니다.

## 7. 공공데이터 API 지역 필터 부재

- **증상** — `areaCd`, `areaCode`, `signguCd` 모두 `INVALID_REQUEST_PARAMETER_ERROR`
- **원인** — 이 엔드포인트에는 지역 필터 파라미터가 없었습니다.
- **해결** — 전국 데이터를 받아 `areaCode == "30"`(대전)으로 걸러냈습니다.
- **추가** — 일일 호출 한도(1,000회)를 넘지 않도록 기간을 분할했습니다.

## 8. 네이버 데이터랩 API 신규 신청 중단

- **증상** — 개발자센터에서 검색어 트렌드 API 신청이 불가했습니다.
- **원인** — 2026-07-31부로 NAVER API HUB로 이관됐습니다.
- **해결** — 과거 데이터는 1회 수집이면 충분하므로 웹사이트 엑셀 다운로드로 전환했습니다.

## 9. Vercel 배포 실패

- **증상** — `No Output Directory named "public" found after the Build completed.`
- **원인** — Build Command를 지정하면 Vercel이 빌드 산출물 폴더(`public`)를 찾습니다.
- **해결** — `frontend/vercel.json`에 `outputDirectory: "."`를 명시했습니다.
- **효과** — 대시보드 설정보다 우선하므로 저장소만 보면 배포 설정을 재현할 수 있습니다.
...
- 라우터는 상태 코드와 예외 변환 같은 HTTP 관심사만 갖습니다.
- 도메인 로직은 서비스에 두어 라우터 없이도 재사용할 수 있게 했습니다.
- 챗봇이 `/api/data/summary`를 HTTP로 다시 부르지 않고 함수를 직접 호출합니다.
- 네트워크 왕복이 사라지고, 같은 로직을 스크립트에서도 씁니다.

## 문서 ID를 연월로 지정한 이유

- Firestore 자동 ID 대신 `2024-05`처럼 연월을 씁니다.
- 같은 달을 두 번 넣는 실수가 구조적으로 불가능해집니다.
- 적재 스크립트를 몇 번 실행해도 중복이 생기지 않습니다.
- `PUT /api/data/2024-05`처럼 URL만 봐도 어떤 데이터인지 압니다.

## 요약에 한계를 함께 담은 이유

- 숫자만 주면 AI는 확신에 찬 문장을 만듭니다.
- "중복 집계", "관심 지표", "집계 기간 차이", "상관≠인과"를 주입했습니다.
- 그 결과 AI가 스스로 단서를 붙여 답합니다.
- 데이터 서비스에서는 잘 말하게 만드는 것보다 틀리지 않게 만드는 것이 중요합니다.

## Firestore 클라이언트를 지연 초기화한 이유

- 앱 기동 시점에 초기화하면 키 설정이 잘못됐을 때 서버 자체가 뜨지 않습니다.
- 첫 요청에서 초기화하면 키가 없어도 `/docs`와 `/health`는 열립니다.
- 배포 직후 원인을 좁히기가 쉬워집니다.

## 목록과 본문 API를 분리한 이유

- 대화 목록에 메시지 전체를 실으면 목록이 무거워집니다.
- 목록은 제목·메시지 수·시간만 줍니다.
- 본문은 `GET /api/conversations/{id}`로 받습니다.
- 미션의 (A)안을 선택한 이유이기도 합니다.

---

# Limitations & Future Work

- 이동통신 방문자 수는 추정치이며 중복 집계가 있습니다. 절대량이 아니라 추세로만 읽어야 합니다.
- 검색 지수는 실제 방문이 아니라 관심을 나타내는 상대 지표입니다.
- 두 지표 모두 증가 추세라 공통 추세가 상관계수를 부풀렸을 수 있습니다.
- 시차 상관과 계절조정 결과를 요약에 추가하면 해석이 더 정확해집니다.
- 중구 비중은 2024년부터만 있어 표본이 30개월입니다.
- 기초 지자체 데이터를 2018년까지 확장하면 비교가 탄탄해집니다.
- 보너스 1(Function Calling + MCP/GPT Actions)은 미구현입니다.
- `get_summary`, `get_data_range`를 도구로 노출하면 특정 구간만 조회해 답하는 흐름을 만들 수 있습니다.
- Render 무료 인스턴스의 콜드스타트는 안내로 대응했을 뿐 근본 해결은 아닙니다.
- 현재 데이터는 대전 한 지역만 다룹니다. 지역을 파라미터로 받으면 다른 도시에도 적용할 수 있습니다.

---

# What We Learned

- 같은 질문이라도 어떤 데이터를 고르느냐가 결론을 바꿉니다.
- 공식 통계라고 해서 시계열로 쓸 수 있는 것은 아니며, 집계 대상 변화부터 확인해야 했습니다.
- 컨텍스트 주입은 숫자를 넣는 일이 아니라 무엇을 근거로 말해야 하는지 알려주는 일이었습니다.
- 사건과 주의사항을 넣은 뒤에야 AI가 쓸모 있는 답을 했습니다.
- 통계는 계산보다 해석 조건을 함께 전달하는 것이 중요했습니다.
- 기간·표본 수·비교 구간이 빠지면 사람도 AI도 오해합니다.
- 배포는 코드가 아니라 설정에서 막힙니다.
- CORS, 출력 디렉터리, 키 경로처럼 로컬에서 드러나지 않는 문제를 먼저 배포해 확인하는 편이 빨랐습니다.
- 키 관리는 규칙을 정하는 것보다 확인하는 절차가 중요했습니다.
- `.gitignore`를 믿지 않고 `git check-ignore`로 매번 확인했습니다.
